...
namespace SinoStation_2025
{
    public partial class RegulatoryReviewForm : System.Windows.Forms.Form
    {
        ...
        private void CreateExcel(Excel.Application excelApp, Excel._Worksheet workSheet, List<ExcelInfo> excelInfoList, List<ElementInfo> oldElemList, List<ElementInfo> elementInfoList)
        {
            // 凍結窗格
            // 1.先選定某一個儲存格當作範圍
            Excel.Range rangeFreezePoint = workSheet.get_Range("A4", "I4");
            // 2.選定此範圍所在的 Sheet
        ...
        private List<ElementInfo> ModelInfo(Document doc, List<string> charsToRemove, List<ExcelCompare> excelCompareList)
        {
            ...
            List<Element> testElems = elems.Where(x => x.Id.Value.Equals((long)2854095)).Select(x => x).ToList();
            foreach (Element elem in elems)
            {
                if (elem is Room)
                {
                    try
                    {
                        ...
                        try
                        {
                            elementInfo.lowerOffset = Math.Round(UnitUtils.ConvertFromInternalUnits(elem.get_Parameter(BuiltInParameter.ROOM_LOWER_OFFSET).AsDouble(), UnitTypeId.Meters), 4, MidpointRounding.AwayFromZero);
                        }
                        catch(Exception ex)
                        {
                            string error = ex.Message + "\n" + ex.ToString();
                        ...
                        // Room的頂部高程
                        double topElevation = Math.Round(UnitUtils.ConvertFromInternalUnits(room.UpperLimit.Elevation, UnitTypeId.Meters), 2, MidpointRounding.AwayFromZero);
                        ...
                        // 儲存長度, 並轉換單位為公尺
                        elementInfo.boundarySegments = maxCurveList.Select(x => Math.Round(UnitUtils.ConvertFromInternalUnits(x.Length, UnitTypeId.Meters), 2, MidpointRounding.AwayFromZero)).OrderByDescending(x => x).ToList();

                        // 儲存Room的LocationPoint
                        LocationPoint lp = elem.Location as LocationPoint;
                        elementInfo.boundaryPoints.Add(lp.Point);
                        elementInfo.maxBoundaryLength = UnitUtils.ConvertFromInternalUnits(maxCurveList.Select(x => x.Length).Max(), UnitTypeId.Meters); // 最長邊界
            ...
            foreach(Document linkDoc in docList)
            {
                ...
                foreach (FamilyInstance door in doors)
                {
                    ...
                    try
                    {
                        ...
                        doorData.height = Math.Round(UnitUtils.ConvertFromInternalUnits(height, UnitTypeId.Millimeters), 0, MidpointRounding.AwayFromZero);
                    }
                    catch (Exception ex)
                    {
                        string error = ex.Message + "\n" + ex.ToString();
                    ...
                    try
                    {
                        ...
                        doorData.width = Math.Round(UnitUtils.ConvertFromInternalUnits(width, UnitTypeId.Millimeters), 0, MidpointRounding.AwayFromZero);
                    }
                    catch (Exception ex)
                    {
                        string error = ex.Message + "\n" + ex.ToString();
                    ...
                    foreach (Floor floor in floors)
                    {
                        ...
                        foreach (Reference getBottomFace in HostObjectUtils.GetBottomFaces(floor))
                        {
                            try
                            {
                                ...
                                // 樓板的底部高程
                                allBottomFaces.bottomElevation = UnitUtils.ConvertFromInternalUnits(floor.get_Parameter(BuiltInParameter.STRUCTURAL_ELEVATION_AT_BOTTOM).AsDouble(), UnitTypeId.Meters);
                    ...
                    foreach (Ceiling ceiling in ceilings)
                    {
                        ...
                        foreach (Reference getBottomFace in HostObjectUtils.GetBottomFaces(ceiling))
                        {
                            try
                            {
                                ...
                                // 天花板的底部高程
                                allBottomFaces.bottomElevation = UnitUtils.ConvertFromInternalUnits(elevation + ceiling.get_Parameter(BuiltInParameter.CEILING_HEIGHTABOVELEVEL_PARAM).AsDouble(), UnitTypeId.Meters);
                                allBottomFacesList.Add(allBottomFaces);
                            }
                            catch (Exception ex)
                            {
                    ...
                    foreach (FootPrintRoof footPrintRoof in footPrintRoofs)
                    {
                        ...
                        foreach (Reference getBottomFace in HostObjectUtils.GetBottomFaces(footPrintRoof))
                        {
                            try
                            {
                                ...
                                double elevation = level.Elevation;
                                // 屋頂的底部高程
                                allBottomFaces.bottomElevation = UnitUtils.ConvertFromInternalUnits(elevation + footPrintRoof.get_Parameter(BuiltInParameter.ROOF_LEVEL_OFFSET_PARAM).AsDouble(), UnitTypeId.Meters);
                                allBottomFacesList.Add(allBottomFaces);
                            }
                            catch (Exception ex)
                            {
                    ...
                    foreach (ExtrusionRoof extrusionRoof in extrusionRoofs)
                    {
                        ...
                        foreach (Reference getBottomFace in HostObjectUtils.GetBottomFaces(extrusionRoof))
                        {
                            try
                            {
                                ...
                                allBottomFaces.bottomElevation = UnitUtils.ConvertFromInternalUnits(elevation, UnitTypeId.Meters);
                                allBottomFacesList.Add(allBottomFaces);
                            }
                            catch (Exception ex)
                            {
                                    ...
                                    foreach (Face face in solid.Faces)
                                    {
                                        if (face is PlanarFace)
                                        {
                                            ...
                                            if (planarFace.FaceNormal.Z < 0)
                                            {
                                                ...
                                                {
                                                    // 樓板、天花板、樓梯的底部高程
                                                    double faceOriginZ = UnitUtils.ConvertFromInternalUnits(planarFace.Origin.Z, UnitTypeId.Meters);
        ...
        private List<double> AllUpBottomFacesDistance(List<AllBottomFaces> allBottomFacesList, XYZ roomPoint)
        {
            List<double> distances = new List<double>();
            // 找到高於roomPoint, z軸的面
            double roomZ = UnitUtils.ConvertFromInternalUnits(roomPoint.Z, UnitTypeId.Meters);
            ...
            foreach(AllBottomFaces upBottomFace in upBottomFaces)
            {
                try
                {
                    ...
                    List<PlanarFace> planarFaces = (from x in upBottomFace.bottomFaces
                                                    where UnitUtils.ConvertFromInternalUnits(x.Origin.Z, UnitTypeId.Meters) > roomZ
                                                    select x).ToList();
                    foreach (PlanarFace planarFace in planarFaces)
                    {
                        try
                        {
                            ...
                            distance = Math.Round(UnitUtils.ConvertFromInternalUnits(distance, UnitTypeId.Meters), 2, MidpointRounding.AwayFromZero);
                            if (distance > 0)
                            {
                ...
                if(rowIndex != -1)
                {
                    ...
                    if (!dgv.Name.Equals("dataGridView3"))
                    {
                        ...
                        if (dgv.Name.Equals("dataGridView5"))
                        {
                            ...
                            {
                                ElementId id = new ElementId(Convert.ToInt64(rowValue));
            ...
            else
            {
                try
                {
                    ...
                    foreach (int areaId in areaIds)
                    {
                        try
                        {
                            ElementId id = new ElementId((long)areaId);
        ...
        private void updateDoorBtn_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView5.Rows)
            {
                ...
                {
                    DoorData doorData = new DoorData();
                    doorData.id = new ElementId(Convert.ToInt64(row.Cells[dataGridView5.ColumnCount - 1].Value.ToString())); // ID
        ...
        public void IdleUpdate(object sender, IdlingEventArgs e)
        {
            ...
            if (elems.Count > 0)
            {
                ...
                {
                    Room room = (Room)elem;
                    if (room.Id.Value != chooseRoomId)
                    {
                        // 找到dataGridView1 Room id的列
                        foreach (DataGridViewRow dgv1Row in dataGridView1.Rows)
                        {
                            if (Convert.ToInt64(dgv1Row.Cells[dgv1Row.Cells.Count - 1].Value.ToString()).Equals(room.Id.Value))
                            {
                        ...
                        {
                            if (Convert.ToInt64(dgv2Row.Cells[dgv2Row.Cells.Count - 1].Value.ToString()).Equals(room.Id.Value))
                            {
                        ...
                        {
                            if (Convert.ToInt64(dgv6Row.Cells[dgv6Row.Cells.Count - 1].Value.ToString()).Equals(room.Id.Value))
                            {
