    ...
    public class FacilityClearance : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if(sinoAR_Form.trueOrFalse == true)
            {
                ...
                using (Transaction trans = new Transaction(doc, "售票機設施淨空"))
                {
                    ...
                    {
                        List<CrushElement> crushElems = crushElements.Where(x => x.elem.Id.Value != crushElement.elem.Id.Value).ToList();
                        foreach (CrushElement crushElem in crushElems)
                        {
                            ...
                            double transDistance = Math.Round(UnitUtils.ConvertFromInternalUnits(distance, UnitTypeId.Meters), 4, MidpointRounding.AwayFromZero);
                ...
                using (Transaction trans = new Transaction(doc, "電扶梯設施淨空"))
                {
                    ...
                    if (view3D != null)
                    {
                        ...
                        double width = Math.Round(UnitUtils.ConvertToInternalUnits(0.85, UnitTypeId.Meters), 4, MidpointRounding.AwayFromZero);
                        double length = Math.Round(UnitUtils.ConvertToInternalUnits(9, UnitTypeId.Meters), 4, MidpointRounding.AwayFromZero);
                            ...
                            try
                            {
                                ...
                                foreach (XYZ arcXYZ in arcXYZs)
                                {
                                    ...
                                    viewPlan = (from x in viewPlans
                                                where x.GenLevel.Id.Value == arczlevel.Id.Value
                                    ...
                                    crushElem.id = escalator.Id.Value; // ID
                ...
                foreach (Face face in solid.Faces)
                {
                    if (face is PlanarFace)
                    {
                        ...
                        if (Math.Round(planarFace.FaceNormal.Z, 2, MidpointRounding.AwayFromZero).Equals(-1))
                        {
                            ...
                            // 線短起點的終點座標, 儲值機3米
                            double length = Math.Round(UnitUtils.ConvertToInternalUnits(3, UnitTypeId.Meters), 2, MidpointRounding.AwayFromZero);
                            ...
                            crushElem.edgeLength = UnitUtils.ConvertFromInternalUnits(startCurve.Length / 2, UnitTypeId.Meters); // 放置點到邊界的長度
        ...
        private void AnalysisGroup(Document doc, List<CrushElement> crushElements)
        {
            ...
            foreach(CrushElement crushElement in crushElements)
            {
                ...
                int a = compareIdElevList.Where(x => x.id.Value.Equals(crushElement.elem.Id.Value)).Where(x => x.elevation.Equals(crushElement.locationPoint.Z)).Count();
                if (a.Equals(0))
                {
        ...
        private List<CompareIdElev> CompareElems(List<CompareIdElev> compareIdElevList, CrushElement crushElement, List<CrushElement> crushElements, Group group)
        {
            ...
            {
                int b = compareIdElevList.Where(x => x.id.Value.Equals(otherCrushElem.elem.Id.Value)).Where(x => x.elevation.Equals(crushElement.locationPoint.Z)).Count();
                if (b.Equals(0))
                {
            ...
            foreach (CrushElement crushElem in crushElems)
            {
                ...
                if (transDistance <= allowDistance)
                {
                    int count = compareIdElevList.Where(x => x.id.Value.Equals(crushElem.elem.Id.Value)).Where(x => x.elevation.Equals(crushElem.locationPoint.Z)).Count();
                    if (count.Equals(0))
                    {
