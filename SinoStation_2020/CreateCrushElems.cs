...
namespace SinoStation_2020
{
    ...
    public class CreateCrushElems : IExternalEventHandler
    {
        ...
        public void Execute(UIApplication uiapp)
        {
            ...
            double elevation = Math.Round(UnitUtils.ConvertToInternalUnits(extrusionHeight, DisplayUnitType.DUT_METERS), 2, MidpointRounding.AwayFromZero);
            ...
            using (Transaction trans = new Transaction(revitDoc, "干涉檢查"))
            {
                ...
                foreach (ElementInfo elementInfo in elementInfoList)
                {
                    ...
                    ElementId elementId = new ElementId(Convert.ToInt32(elementInfo.id));
        ...
        private List<CrushElemInfo> CrushReport(Document doc, LogicalOrFilter logicalOrFilter)
        {
            ...
            foreach (DirectShape directShape in genericModels)
            {
                        ...
                        foreach (Element boxFilterElem in boxFilterElems)
                        {
                            try
                            {
                                ...
                                if (elems.Count > 0)
                                {
                                    ...
                                    {
                                        if (elem.Category.Id.IntegerValue == (int)BuiltInCategory.OST_Stairs) // 樓梯
                                        {
                                            Stairs stairs = elem as Stairs;
                                            if(stairs != null) { crushElemInfo.crushStairs.Add(stairs); }                                            
                                        }
                                        else if (elem.Category.Id.IntegerValue == (int)BuiltInCategory.OST_GenericModel) // 一般模型
                                        {
                                        ...
                                        {
                                            if(elem.Category.Id.IntegerValue == (int)BuiltInCategory.OST_Floors) // 樓板
                                            {
                                                if(elem is Floor)
                                                {
                                            ...
                                            else if (elem.Category.Id.IntegerValue == (int)BuiltInCategory.OST_Ceilings) // 天花板
                                            {
                                                if(elem is Ceiling)
                                                {
                ...
                try
                {
                    ElementId crushElemId = new ElementId(Convert.ToInt32(crushElemInfo.id));
                    ...
                    if (crushElemInfo.crushCeilings.Count > 0 && crushElemInfo.crushFloors.Count > 0)
                    {
                        ...
                        double clearSpace = Math.Round(UnitUtils.ConvertFromInternalUnits(floorHeight - ceilingHeight, DisplayUnitType.DUT_METERS), 2, MidpointRounding.AwayFromZero);
                    ...
                    else if (crushElemInfo.crushCeilings.Count > 0 && crushElemInfo.crushFloors.Count.Equals(0))
                    {
                        ...
                        double clearSpace = extrusionHeight - Math.Round(UnitUtils.ConvertFromInternalUnits(ceilingHeight - lp.Point.Z, DisplayUnitType.DUT_METERS), 2, MidpointRounding.AwayFromZero);
                        if (clearSpace > 0) { crushElemInfo.clearSpace = Math.Round(clearSpace, 2, MidpointRounding.AwayFromZero); }
                    }
                    // 淨高
                    if (crushElemInfo.crushElems.Count > 0)
                    {
                        ...
                        if (crushElemInfo.crushCeilings.Count > 0)
                        {
                            ...
                            crushElemInfo.clearSpaceHeight = Math.Round(UnitUtils.ConvertFromInternalUnits(ceilingHeight - room.UpperLimit.Elevation, DisplayUnitType.DUT_METERS), 2, MidpointRounding.AwayFromZero);
                        }
                        else
                        {
                        ...
                        if (bottomFace is PlanarFace)
                        {
                            ...
                            try
                            {
                                ...
                                if(xyzs.Count > 0)
                                {
                                    ...
                                    double lowestFaceHeight = Math.Round(UnitUtils.ConvertFromInternalUnits(bottomFaceHeight - roomHeight, DisplayUnitType.DUT_METERS), 2, MidpointRounding.AwayFromZero);
